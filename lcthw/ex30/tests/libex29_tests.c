...
int check_function(const char *func_to_run, const char *data, int expected)
{
    ...
    check(func != NULL, "check_function", "Did not find %s function in the library %s: %s", func_to_run, lib_file, dlerror());    

    int rc = func(data);
    check(rc == expected,"check_function", "Function %s return %d for data: %s", func_to_run, rc, data);
